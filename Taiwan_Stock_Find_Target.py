...
split_list = []
print(f"待分析股票總數: {len(filtered_stock_list)}")
for i in range(1500,1512):  # @@修改的地方
...
#url相關變數
url_prev = 'https://s.yimg.com/nb/tw_stock_frontend/scripts/TaChart/tachart.30616e0d1d.html?sid='
url_post = '&w=660'
...
for stock_code in split_list:
    ...
    if(not re.search('.*No\sData$',soup.text)):
        ...
        if(counter == 10):
            ...
            counter = 0
            print('批次輸出:')
            for _,v in collected_stocks.items():
                print(f"{v}")
...
for _,v in sorted_collected_stocks:
    print(f"{v}")
